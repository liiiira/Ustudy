...
export async function refresh(refreshToken: string): Promise<string> {
  ...
  if (!userRefreshToken) throw new AppError("Invalid Refresh Token", 401);
  ...
  const validToken: boolean = tokenUtils.compareRefreshToken(
    refreshToken,
...
export async function revokeRefreshToken(
  ...
  const result: UserToken | undefined =
    await authRepository.revokeRefreshToken(hashedRefreshToken);

  if (!result) throw new AppError("Token was not found in DB", 500);
...
async function updateRefreshToken(userId: string): Promise<string> {
  ...
  const token = await authRepository.upsertRefreshToken({
    userId,
    expiresAt,
    hashedToken,
  });

  if (!token) throw new AppError("Unexpected Database Error", 500);
