...
export async function create(
  ...
  const createdComment: CommentJoinUser = await commentService.create(
    ownerId,
    postId,
    commentInput,
  );
...
export async function findAllPost(
  ...
  const postComments: CommentJoinUser[] =
    await commentService.findAllPost(postId);
...
export async function updateById(
  req: Request<{ communityId: string; postId: string; commentId: string }>,
  res: Response,
) {
  ...
  const updatedComment: CommentDB | null = await commentService.updateById(
    userId,
    commentId,
    { textContent },
  );
...
export async function deleteById(
  ...
  const deletedComment: { id: string } = await commentService.deleteById(
    userId,
    commentId,
  );
