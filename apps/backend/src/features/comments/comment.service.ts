...
): Promise<CommentJoinUser> {
  const postExists: Post | null = await postService.findById(postId);

  if (!postExists) throw new AppError("Post was not found", 404);

  const createdComment: CommentJoinUser | null = await commentRepository.create(
    { ...commentInput, postId, ownerId },
  );

  if (!createdComment)
    throw new AppError(
      "Unexpected Internal Error: Failed to create comment",
      500,
    );
...
export async function findAllPost(postId: string): Promise<CommentJoinUser[]> {
  ...
  const postComments: CommentJoinUser[] =
    await commentRepository.findAllPost(postId);
...
export async function updateById(
  ...
  if (!comment) throw new AppError("Comment doesn't exist", 404);

  if (userId !== comment.ownerId)
    throw new AppError("You are not allowed to update this comment", 403);

  if (!textContent) throw new AppError("Body is Empty", 400);
  ...
  const updatedComment: CommentDB | null = await commentRepository.updateById(
    commentId,
    { textContent },
  );
  ...
  if (!updatedComment)
    throw new AppError(
      "Unexpected Internal Error: Failed to update comment",
      500,
    );
...
export async function deleteById(userId: string, commentId: string) {
  ...
  if (!comment) throw new AppError("Comment was not found", 404);
