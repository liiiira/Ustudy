...
export async function create(
  ...
  if (nameExists) throw new AppError("Community name already taken", 409);

  let uploadId: string | undefined;

  if (imageUrl) {
    const upload = await uploadService.verifyUploadOwnerShip(ownerId, {
      communityUrl: imageUrl,
    });
    uploadId = upload.id;
  }

  const createdCommunity: CommunityDB | null = await communityRepository.create(
    { ownerId, name, description, uploadId },
  );
...
export async function getByIdJoinUser(id: string): Promise<CommmunityJoinUser> {
  ...
  if (!foundCommunity) throw new AppError("Community was not found", 404);
...
export async function updateById(
  ...
  if (!community) throw new AppError("Community doesn't exist", 404);
  ...
  const updatedCommunity: CommunityDB | null =
    await communityRepository.updateById(id, modifiedAttributes);
...
export async function delelteById(
  userId: string,
  id: string,
): Promise<{ id: string }> {
  const community: CommunityDB | null = await findById(id);

  if (!community) throw new AppError("Community was not found", 404);

  if (community.ownerId !== userId)
    throw new AppError("You are not allowed to delete this community", 403);

  const deletedCommunity: { id: string } | null =
    await communityRepository.deleteById(id);

  if (!deletedCommunity) throw new AppError("User Not Found", 404);
