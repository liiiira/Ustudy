...
export async function createOrFindConversation(
  ...
  switch (body.type) {
    case "direct":
      return findOrCreateDirect(requesterId, body.otherUserId);
    case "group":
      return createGroup(requesterId, {
...
async function findOrCreateDirect(
  requesterId: string,
  otherUserId: string,
): Promise<{ created: boolean; conversation: DirectConversation }> {
  ...
  const response = await conversationRepository.findOrCreateDirect({
    directKey,
    requesterId,
    otherUserId,
  });
...
async function createGroup(
  requesterId: string,
  { memberIds, name, imageUrl }: GroupConversationInput,
): Promise<{ created: boolean; conversation: GroupConversation }> {
  const ownerId = requesterId;
  const uniqueMemberIds: string[] = [...new Set([...memberIds, ownerId])];
  const existingUserIds: string[] =
    await userService.findExistingIds(uniqueMemberIds);
  const missingUserIds: string[] = uniqueMemberIds.filter(
  ...
  if (imageUrl) {
    const upload = await uploadService.verifyUploadOwnerShip(ownerId, {
      conversationUrl: imageUrl,
    });
    uploadId = upload.id;
  }

  const groupConversation: GroupConversation | null =
    await conversationRepository.createGroup({
  ...
  if (!groupConversation)
    throw new AppError(
      "Couldn't create group conversation due to internal error",
      500,
    );
...
export async function getById(
  ...
  if (!member)
    throw new AppError("You are not a member of this Conversaiton", 403);
...
export async function updateById(
  ...
  const requesterMember: ConversationMember | null = await findMember(
    conversationId,
    requesterId,
  );
  if (!requesterMember)
  ...
  switch (conversation.type) {
    case "group":
      return updateGroup(
        requesterMember,
        conversation as GroupConversation,
        body,
      );
...
async function updateGroup(
  requesterMember: ConversationMember,
  group: GroupConversation,
  { name, imageUrl }: ConversationUpdate,
): Promise<GroupConversation> {
  if (!name && !imageUrl) throw new AppError("Nothing to update", 400);

  if (requesterMember.role !== "admin")
    throw new AppError("Only an admin can update this conversation", 403);

  let uploadId: string | undefined;
  if (imageUrl) {
    const upload = await uploadService.verifyUploadOwnerShip(
  ...
  const updatedGroup: GroupConversation | null =
    await conversationRepository.updateGroup(group.id, { uploadId, name });
  if (!updatedGroup)
...
async function updateDirect(
  _requesterMember: ConversationMember,
  _direct: DirectConversation,
  _body: ConversationUpdate,
...
export async function deleteGroup(
  ...
  if (requesterMember.role !== "admin")
    throw new AppError(
      "Only and admin can delete this group conversation",
      403,
    );

  const response = await conversationRepository.deleteGroup(
    groupConversation.id,
  );
  if (!response)
    throw new AppError(
      "Failed to delete group conversation due to unexpected error",
      500,
    );
...
export async function addMembers(
  ...
  if (requesterMember.role !== "admin")
    throw new AppError(
      "Only an admin can add members to this conversation",
      403,
    );
  ...
  const addedMembers: string[] = await conversationRepository.addMembers(
    conversationId,
    uniqueMemberIds,
  );
...
export async function removeMember(
  ...
  const targetMember: ConversationMember | null =
    requesterId === memberId
      ? requesterMember
      : await findMember(conversationId, memberId);
  if (!targetMember)
    throw new AppError("Member not found in this conversation", 404);

  if (targetMember.role === "admin")
    throw new AppError("An admin cannot be removed from the conversation", 400);
  ...
  const removed = await conversationRepository.removeMember(
    conversationId,
    memberId,
  );
  if (!removed)
    throw new AppError(
      "Member was not removed due to an unexpected error",
      500,
    );
