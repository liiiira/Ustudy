...
export default function FormField({id, name, type,  value, placeholder, handleChange, inputError,  charLimit} : FieldProps){
      ...
      <div className="flex flex-col gap-1">
        ...
        <input className={`bg-white px-4 py-2 border-2 border-gray-300 rounded-md outline-none transition-colors
          ...
          ${value.length > 0  && inputError.length > 0 ?  "focus:ring-2 focus:ring-red-500 border-red-500 bg-red-50" : ""}`}
            id={id} type={type} name={name} placeholder={placeholder}  onChange={handleChange} maxLength={charLimit} required />
