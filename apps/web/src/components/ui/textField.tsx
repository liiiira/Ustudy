...
export default function TextField({id, name,  value, placeholder, label, handleChange, inputError,  maxLength, rows}: TextFieldProps){
  return (
    <div className="flex flex-col gap-1">
      ...
      <textarea className={`bg-white px-4 py-2 border-2 border-gray-300 rounded-md outline-none transition-colors
          ...
          ${value.length > 0  && inputError.length > 0 ?  "focus:ring-2 focus:ring-red-500 border-red-500 bg-red-50" : ""}`}
            id={id} name={name} placeholder={placeholder}  onChange={handleChange} rows={rows} maxLength={maxLength} required />

        <div className="flex flex-row items-center justify-between"> 
          <div className=" min-h-5 text-red-500 text-xs">{value.length > 0 && inputError[0]}</div>
          <div className="text-sm text-gray-500 font-medium">{value.length}/{maxLength}</div>
        </div>
    </div>
  )
}
