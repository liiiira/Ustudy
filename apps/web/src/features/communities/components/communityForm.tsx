...
export default function CommunityForm({
  ...
  const [community, setCommunity] = useState<CreateCommunityData>({
    name: name,
    description: description,
    imageUrl: imageUrl,
  });
  const [apiError, setApiError] = useState<string>("");
  const [inputError, setInputError] = useState<CreateCommunityError>({
    name: [],
    description: [],
  });
  ...
  async function handleImageSelect(file: File) {
    ...
    if (url)
      setCommunity((prev: CreateCommunityData): CreateCommunityData => ({
        ...prev,
        imageUrl: url,
      }));
  }
  ...
  function validateCommunity(community: CreateCommunityData): boolean {
    ...
    const descriptionErrors: string[] = validateLength(
    ...
    return ![nameErrors, descriptionErrors].some(
  ...
  async function handleSubmit(e: React.SubmitEvent) {
    ...
    try {
      if (mode === "Create") await communityApi.create(community);
      else if (mode === "Update") await communityApi.updateById(id!, community);
