    ...
    <div className="bg-white px-8 pt-4 pb-10 flex flex-col gap-4 rounded-2xl w-full h-max shadow-xl">
      <div className="flex w-full flex-row justify-between">
        <div className="flex flex-row gap-2 justify-center items-center font-light text-xs">
          <div>published by: {ownerName}</div>
      ...
      <div className=" px-2 py-1 flex flex-col h-max gap-4">
        <div className="font-bold text-black text-xl whitespace-pre-wrap wrap-break-word">
          {title}
        </div>
        <div className="font-light text-black whitespace-pre-wrap wrap-break-word">
          {textContent}
        </div>
      </div>

      {imageUrl && (
        <img src={imageUrl} className="w-full h-auto object-cover rounded" />
      )}
    </div>
  );
}
