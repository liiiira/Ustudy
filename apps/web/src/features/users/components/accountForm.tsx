...
export default function AccountForm({
  ...
  const [userError, setUserError] = useState<UpdateUserAccountErrors>({
    username: [],
  });
  ...
  function handleChange(e: React.ChangeEvent<HTMLInputElement>) {
    const newUser: UpdateUserAccount = {
      ...user,
  ...
  async function handleAvatarSelect(file: File) {
    ...
    if (url)
      setUser((prev: UpdateUserAccount): UpdateUserAccount => ({
        ...prev,
        avatarUrl: url,
      }));
  }
  ...
  return (
    <form
      className="w-full px-4 py-2 flex flex-col gap-2 rounded-xl bg-white"
      onSubmit={handleSubmit}
    >
      <div className="text-lg font-bold">Account preferences</div>
      ...
      <div className="w-full flex flex-row-reverse">
        <Button type="submit" variant="Primary" disabled={!valid || uploading}>
          Apply Changes
        </Button>
      </div>
    </form>
